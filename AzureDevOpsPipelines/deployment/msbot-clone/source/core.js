...
class Core {
    ...
    botClone() {
        ...
        var inputs = this.GetParameters(this.parametersClone);
        this.run(`msbot clone services ` +
            `--subscriptionId ${this.subscriptionID}` +
            ` ${inputs} ` +
            `--force ` +
            `--verbose`, this.getCWD('code-dir'));
    }
    getCWD(cwd) {
    ...
    run(command, options) {
        var command = `${this.getPrefix()} ${command}`;
